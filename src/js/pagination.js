...
export default class Pagination {
  constructor({ currentPage, totalPages } = {}) {
    ...
    this._totalPages = Math.floor(totalPages / (this._currentPage * 20));
  ...
  createPagination() {
    if (window.innerWidth > 320 && window.innerWidth < 768) {
      if (this._currentPage > 1) {
        this.markup += `<li class="pagination__item pagination-item--btn decrement"> <svg class="paginate-btn__icon"><use href="${sprite}#icon-arrow-left"></use></svg></li>`;
      }
      if (this._currentPage > 1) {
        this.markup += `<li class="pagination__item ">1</li>`;
      }

      if (this._currentPage > 3) {
        this.markup += `<li class="pagination__item ">${this.beforeTwoPage}</li>`;
      }

      if (this._currentPage > 2) {
        this.markup += `<li class="pagination__item ">${this.beforePage}</li>`;
      }
      this.markup += `<li class="pagination__item current"><b>${this._currentPage}</b></li>`;
      if (this._totalPages - 1 > this._currentPage) {
        this.markup += `<li class="pagination__item ">${this.afterPage}</li>`;
      }
      if (this._totalPages - 2 > this._currentPage) {
        this.markup += `<li class="pagination__item ">${this.afterTwoPage}</li>`;
      }
      if (this._totalPages > this._currentPage) {
        this.markup += `<li class="pagination__item ">${this._totalPages}</li>`;
        this.markup += `<li class="pagination__item pagination-item--btn increment"> <svg class="paginate-btn__icon"><use href="${sprite}#icon-arrow-right"></use></svg><li>`;
      }
    } else {
      if (this._currentPage > 1) {
        this.markup += `<li class="pagination__item pagination-item--btn decrement"> <svg class="paginate-btn__icon"><use href="${sprite}#icon-arrow-left"></use></svg></li>`;
